...
for rep=1:reps
    ...
    while FES<fit
       ...
       %%Please note that:In our work, we start from the basic multi task MFEA algorithm of single target 
       %%and extend it to G-MFEA algorithm. Due to the limitation of the length of the paper, only the related 
       %%contents of the latter are published. (see the paper for details)
        
        % GMFEA optimization
        data_GMFEA=GMFEA(Tasks,pop,gen,rmp,sub_pop2,mu,mum);
        % MFEA optimization
%         data_GMFEA=MFEA(Tasks,pop,gen,rmp,sub_pop2,mu,mum);
